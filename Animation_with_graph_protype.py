...
class Simulation:
                    ...
                    for building in range(len(self.buildinglist)):
                        if self.overlap([x_school-(school_size[0]/2), x_school+(school_size[0]/2)],
                                        ...
                                        [self.buildinglist[building].position[1]-(self.buildinglist[building].size[1]/2),
                                         self.buildinglist[building].position[1]+(self.buildinglist[building].size[1]/2)]):
                            overlapping_building = True
                            print('school on building yikes')
                            break
                ...
                if overlapping_internal == False and overlapping_building == False:
                    for key_building in range(len(self.key_buildinglist)):
                        if self.overlap([x_school-(school_size[0]/2), x_school+(school_size[0]/2)],
                            ...
                            print('school on key building yikes')
                            break
                    ...
                    for key_building in range(len(self.key_buildinglist)):
                        if self.overlap([x_house-(house_size[0]/2), x_house+(house_size[0]/2)],
                                        ...
                                        [self.key_buildinglist[key_building].position[1]-(self.key_buildinglist[key_building].size[1]/2),
                                         self.key_buildinglist[key_building].position[1]+(self.key_buildinglist[key_building].size[1]/2)]):
                            overlapping_key_building = True
                            print('house on key building yikes')
                            break
    ...
    def update(self):
        ...
        self.percentage = np.sum(infected_list)/len(infected_list)
        self.infection_list.append(np.sum(infected_list))
        self.info()
...
#for graph
#ax2 = fig.add_subplot(212,aspect='equal', autoscale_on=False)  
line1, = ax2.plot([], [], lw=2)
line = [line1]
...
def init():
    ...
    global sim, rect
    sim = Simulation([50, 50], 10, 300, 80, 1)
    sim.infection_list.append(np.sum([1 for i in sim.personlist if i.infection_status==1]))
...
def animate(i):
    ...
    xdata.append(sim.graph_time)
    ydata.append(sim.percentage)
    ax2.set_xlim(0,sim.graph_time)
